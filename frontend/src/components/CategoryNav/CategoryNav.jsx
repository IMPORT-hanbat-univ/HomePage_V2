            ...
            <ul>
                {categoryList&&categoryList.length > 0&&categoryList.map((category) => (
                    <li key={category} className='py-2 px-3'>
                        <Link className={cls('text-base cursor-pointer', {"text-lg font-semibold text-import-color" : category === seletedCategory })} href={{ pathname, query: { ...query, category: category } }}>
                            {category}
                        </Link>
                    </li>
                ))}
